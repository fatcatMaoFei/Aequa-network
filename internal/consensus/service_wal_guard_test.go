...
func (s *guardStore) SaveLastState(_ context.Context, ls state.LastState) error {
	atomic.AddInt32(&s.saves, 1)
	return nil
}
func (s *guardStore) LoadLastState(_ context.Context) (state.LastState, error) {
...
func TestService_WAL_Guard_DropsOlderIntents(t *testing.T) {
	...
	b.Publish(ctx, bus.Event{Kind: bus.KindConsensus, Height: 4, Round: 1, Body: qbft.Message{ID: "c4", From: "n1", Type: qbft.MsgCommit, Height: 4, Round: 1}})
	time.Sleep(30 * time.Millisecond)

	if atomic.LoadInt32(&st.saves) != 0 {
		t.Fatalf("older intent should be dropped, got saves=%d", st.saves)
	}
}
