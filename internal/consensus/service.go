...
type QbftBroadcaster interface {
...
type TSSAggVerifier interface {
	VerifyAgg(pkGroup []byte, msg []byte, aggSig []byte) bool
}
...
type TSSSigner interface {
	Sign(ctx context.Context, height, round uint64, msg []byte) ([]byte, error)
}
...
func (s *Service) Start(ctx context.Context) error {
	...
	if s.v == nil {
		s.v = qbft.NewBasicVerifierWithPolicy(qbft.DefaultPolicy())
	}
	if s.store == nil {
