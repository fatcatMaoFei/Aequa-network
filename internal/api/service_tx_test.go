...
func TestHandleTxPlain_OK_PublishAndBroadcast(t *testing.T) {
	...
	wtx := wire.TxEnvelope{Type: wire.TypePlaintextV1, From: "A", Nonce: 0, Gas: 21000, Fee: 100, Sig: bytes.Repeat([]byte{1}, 32)}
...
func TestHandleTxPlain_InvalidTx(t *testing.T) {
	s := &Service{addr: ":0"}
	bad := wire.TxEnvelope{Type: wire.TypePlaintextV1, From: "", Nonce: 0, Gas: 0, Fee: 0, Sig: []byte{1, 2, 3}}
