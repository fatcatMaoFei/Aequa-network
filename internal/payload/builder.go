...
func PrepareProposal(c *Container, hdr BlockHeader, pol BuilderPolicy) StandardBlock {
    ...
    for _, typ := range pol.Order {
        if remain <= 0 { break }
        got := c.GetN(typ, remain, pol.BudgetBytes)
        for _, p := range got {
...
func ProcessProposal(b StandardBlock, pol BuilderPolicy) error {
    if len(pol.Order) == 0 { return nil }
    // build priority map
    pri := map[string]int{}
    for i, t := range pol.Order { pri[t] = i }
    lastPri := -1
    ...
    for _, it := range b.Items {
        t := it.Type()
        p, ok := pri[t]
        if !ok { return errors.New("unexpected payload type: " + t) }
        if p < lastPri { return errors.New("type priority violated") }
        if prev, seen := lastKey[t]; seen {
            if it.SortKey() > prev { return errors.New("sortkey not non-increasing for type: " + t) }
        }
        lastKey[t] = it.SortKey()
        if p > lastPri { lastPri = p }
    }
    return nil
}
