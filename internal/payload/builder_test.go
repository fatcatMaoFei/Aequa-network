...
func TestPrepareProposal_RejectsBelowThresholds(t *testing.T) {
	bidPool := auction.New()
	ptPool := pt.New()
	...
	_ = c.Add(&auction.AuctionBidTx{From: "A", Nonce: 0, Gas: 1, Bid: 5, FeeRecipient: "r", Sig: make([]byte, 32)})
	_ = c.Add(&auction.AuctionBidTx{From: "B", Nonce: 0, Gas: 1, Bid: 15, FeeRecipient: "r", Sig: make([]byte, 32)})
	_ = c.Add(&pt.PlaintextTx{From: "C", Nonce: 0, Gas: 1, Fee: 1, Sig: make([]byte, 32)})
	_ = c.Add(&pt.PlaintextTx{From: "D", Nonce: 0, Gas: 1, Fee: 12, Sig: make([]byte, 32)})
	pol := payload.BuilderPolicy{
		Order: []string{"auction_bid_v1", "plaintext_v1"},
		MaxN:  4, MinBid: 10, MinFee: 10,
	}
	blk := payload.PrepareProposal(c, payload.BlockHeader{Height: 1, Round: 1}, pol)
	if len(blk.Items) != 2 {
